...
/** Allocate a new inode on memory. */
bool
...
/** Grow the inode to the new length.
   Returns the new length if successful, -1 otherwise. */
off_t
inode_grow (struct inode *inode, off_t length)
{
  ...
  while (inode->direct_index < (int) DIRECT_BLOCKS + INDIRECT_BLOCKS && grow_sectors != 0)
  {
    ...
    {
      if (!free_map_allocate(1, &blocks[inode->indirect_index])) {
        return -1;
      }
...
/** Return the number of openers of INODE. */
int
...
/** Return the parent block sector of INODE. */
block_sector_t
...
/** Set the parent block sector of the inode of child block sector to INODE. */
bool
